...
import { EntityArrayResponseType, ShipmentService } from '../service/shipment.service';
...
export class ShipmentComponent implements OnInit {
  ...
  trackId = (_index: number, item: IShipment): number => this.shipmentService.getShipmentIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.shipments = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IShipment[] | null): IShipment[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.shipmentService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
